...
export function parseM3u8(text, baseUrl) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (line.startsWith("#EXT-X-MAP:")) {
      ...
      playlist.mapUri = attrs.URI ? resolveUri(attrs.URI, baseUrl) : "";
      playlist.unsupportedReason = "检测到 EXT-X-MAP / fMP4，当前只支持 MPEG-TS 分片。";
      continue;
    }
    ...
    playlist.segments.push({
      ...
      mediaSequence: seq,
      key: { ...currentKey, iv: currentKey.iv ? new Uint8Array(currentKey.iv) : null },
      byteRange: range,
    });
    mediaIndex += 1;
    pendingInf = null;
...
export function summarizePlaylist(playlist) {
  const methods = new Set(playlist.segments.map((s) => s.key.method));
