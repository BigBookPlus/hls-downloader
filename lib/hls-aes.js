...
export function looksLikeFmp4(bytes) {
  if (!bytes || bytes.length < 8) return false;
  const type = String.fromCharCode(bytes[4], bytes[5], bytes[6], bytes[7]);
  return FMP4_BOX_TYPES.has(type);
}

export function looksLikeHlsMedia(bytes) {
  return looksLikeMpegTs(bytes) || looksLikeFmp4(bytes);
}
...
function acceptDecryptedMedia(bytes) {
  if (looksLikeMpegTs(bytes)) return trimToMpegTs(bytes);
  if (looksLikeFmp4(bytes)) return bytes;
  return null;
}
...
export async function decryptAes128Cbc(keyBytes, iv, cipherBytes) {
  ...
  try {
    ...
    const accepted = acceptDecryptedMedia(web);
  ...
  const accepted = acceptDecryptedMedia(candidate) || acceptDecryptedMedia(raw);
  if (accepted) return accepted;
  throw new Error(
    "解密后不是有效 MPEG-TS 或 fMP4。key/IV 可能错误、过期，或这不是 AES-128 分片。未写入密文拼接文件。"
  );
}
